    ...
    <Link href={`/product/${p.slug}`} className="group block" data-cursor>
      <div className="card-img aspect-[4/5]">
        ...
        <button
          ...
          className="absolute bottom-4 left-1/2 flex -translate-x-1/2 translate-y-4 items-center gap-2 rounded-full bg-cream/95 px-6 py-3 text-[11px] font-black uppercase tracking-[0.18em] text-ink opacity-0 shadow-lg backdrop-blur transition-all duration-400 group-hover:translate-y-0 group-hover:opacity-100 hover:bg-ink hover:text-cream"
        >
      ...
      <div className="mt-4 flex items-start justify-between gap-3 px-1">
        <div>
          ...
          <h3 className="font-display mt-1 text-[19px] font-semibold leading-snug transition-colors duration-300 group-hover:text-rose-deep">
            {p.name}
          </h3>
          <p className="mt-0.5 text-[13px] text-ink-soft">{p.tagline}</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="font-display text-[19px] font-semibold">{inr(p.price)}</p>
