...
const STEP_LABEL: Record<string, string> = { pending: "placed", confirmed: "confirmed", packed: "packed", out_for_delivery: "on your way", delivered: "delivered" };
  ...
  return (
    <section className="wrap max-w-[860px] pb-24 pt-12">
      ...
      <div className="rounded-[32px] border border-line bg-white/70 p-8 backdrop-blur md:p-10">
            ...
            {STATUS_STEPS.map((s, i) => (
              <div key={s} className="flex flex-1 items-center last:flex-none">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className={"text-[10px] font-black uppercase tracking-wider " + (i <= stepIdx ? "text-rose" : "text-ink-soft/60")}>{STEP_LABEL[s] ?? s}</span>
                </div>
        ...
        {o.payment_method === "UPI" && o.payment_status !== "paid" && o.status !== "cancelled" && (
          <OrderPay orderId={o.id} total={o.total} upiId={settings.upiId} payeeName={settings.payeeName} code={orderCode(o.id)} />
        )}

        {(() => {
          const otpEntry = [...(o.timeline ?? [])].reverse().find((t: { otp?: string }) => t.otp) as { otp?: string } | undefined;
          if (!otpEntry?.otp) return null;
          return (
            <div className="mt-7 rounded-2xl bg-butter/70 p-5 text-sm">
              <p className="font-bold">Your delivery OTP: <span className="ml-1 rounded-lg bg-white px-3 py-1 font-mono text-xl font-black tracking-[0.3em]">{otpEntry.otp}</span></p>
              <p className="mt-1 text-[12px] leading-relaxed text-ink-soft">
                Our delivery partner will ask for this 4-digit code at your door — it proves the hamper reached the right hands.
                {o.status === "delivered" ? " This order was delivered and OTP-verified. Thank you!" : " Keep it handy until delivery."}
              </p>
            </div>
          );
        })()}
