...
import { notifyOwners, notifyCustomer } from "@/lib/notify";
import type { Coupon, Product } from "@/lib/types";

export const dynamic = "force-dynamic";

// simple per-IP order rate limit (bots / accidental double-submits)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const t = Date.now();
    const arr = (hits.get(ip) || []).filter((x) => t - x < 3600_000);
    arr.push(t);
    hits.set(ip, arr);
    if (arr.length > 12) return NextResponse.json({ ok: false, error: "Too many orders from this connection in one hour — please WhatsApp us at +91 76688 19833." }, { status: 429 });
    ...
    // real stock control — refuse overselling, then decrement after save
    for (const l of lines) {
      if ((l.p.stock ?? 0) < l.qty)
        return NextResponse.json({ ok: false, error: `“${l.p.name}” has only ${l.p.stock ?? 0} piece(s) left — please reduce the quantity.` }, { status: 409 });
    }
    ...
    for (const l of lines) {
      await sb.from("products").update({ stock: Math.max(0, (l.p.stock ?? 0) - l.qty) }).eq("id", l.p.id);
    }
    ...
    const upiId = String(settings?.upiId || "tajgifts@upi");
    const upiLink =
      `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(String(settings?.payeeName || "Taj Gifts"))}` +
      `&am=${total.toFixed(2)}&cu=INR&tn=${encodeURIComponent("Taj" + orderId.slice(0, 8))}`;
    notifyCustomer(orderRow as never, "placed").catch(() => {});

    return NextResponse.json({ ok: true, id: order.id, total, upiLink });
  } catch (e) {
