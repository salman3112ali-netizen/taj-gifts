        ...
        <fieldset className="mt-10">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <button type="button" onClick={() => setPay("UPI")} className={"rounded-2xl border-2 p-5 text-left transition " + (pay === "UPI" ? "border-rose bg-blush/40" : "border-ink/10 bg-white/60 hover:border-ink/25")}>
              <p className="font-bold">📲 UPI transfer</p>
              <p className="mt-1 text-[13px] text-ink-soft">{settings.upiId} · we confirm on WhatsApp in minutes.</p>
            </button>
          </div>
          {pay === "UPI" && (
            <div className="mt-4 rounded-2xl bg-lav/70 p-5 text-[14px] leading-relaxed">
              <p className="font-bold">Pay {inr(total)} to <span className="rounded bg-white px-2 py-0.5 font-mono">{settings.upiId}</span> ({settings.payeeName})</p>
              <p className="mt-1 text-ink/70">After placing the order, send the payment screenshot on WhatsApp — your hamper enters the tying queue the moment it lands.</p>
            </div>
          )}
        </fieldset>
      ...
      <aside className="lg:sticky lg:top-28 lg:self-start">
        <div className="rounded-[28px] border border-line bg-white/70 p-7 backdrop-blur">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center text-[10px] font-black uppercase tracking-wider text-ink-soft">
            <span className="rounded-xl bg-cream-deep px-2 py-2.5">Hand-tied</span>
            <span className="rounded-xl bg-cream-deep px-2 py-2.5">COD · UPI</span>
            <span className="rounded-xl bg-cream-deep px-2 py-2.5">Note included</span>
          </div>
        </div>
      </aside>
    </section>
